    ...
    function replaceURLWithHyperlinks(text) {
        ...
        if (_nodeIndex != -1) {
            ...
            if(articlesText.length){
                $('<h4>').text("Articles ("+articlesText.split(";").length+")").appendTo(_html);
                for (var i = 0;i <  articlesText.split(";").length; i++){
                    ...
                    $("<a>")
                        ...
                        .attr("href", "#")
                        .mouseover(function () {
                            GexfJS.params.activeArticle = $(this).text();
                            GexfJS.params.activeNode = _e.t;
                            return false;
                        })
                        .click(function () {
            ...
            if (_str_undir.length) {
                $('<h4>').text(strLang("undirLinks")+" ("+_str_undir.length+")").appendTo(_html);
            ...
            success: function (data) {
                ...
                if (isJson) {
                    ...
                    $(_nodes).each(function () {
                        ...
                        _d.G = "rgba(" + Math.floor(84 + .33 * _r) + "," + Math.floor(84 + .33 * _g) + "," + Math.floor(84 + .33 * _b) + ",.3)";
                        _d.a = [];
                        $(_attr).each(function () {
                            var _a = $(this),
                                _for = _a.attr("for");
    ...
    function traceMap() {
        ...
        //===============================================================================================================================
            ...
            for (var i in GexfJS.graph.edgeList) {
                ...
                var _isLinked = false;
                if (_centralNode != -1 && !GexfJS.params.activeArticle.length) {
                ...
                //===============================================================================================================================
                //IF IT HAS A CENTRAL ARTICLE THEN DRAW ITS LINES
                _temp = GexfJS.graph.nodeList[0];
                articlesID = 0;
                ...
                if(GexfJS.params.activeArticle.length)
                {
                    currentDsText = _ds.a[articlesID][1].split(";");
                    currentDtText = _dt.a[articlesID][1].split(";");
                    authorT =_dt.a[3][1]
                    authorS = _ds.a[3][1]

                    sourceHasArticle = false;   
                    for(var k = 0; k < currentDsText.length;k++)
                    {
                        currentDsText[k] = currentDsText[k].replace(/^\s+/g, '');
                        cleanedArticle = GexfJS.params.activeArticle.replace(/^\s+/g, '');
                        if(currentDsText[k] ==  cleanedArticle)
                        {
                            debugger
                            _tagsMisEnValeur.push(_six);
                    ...
                    for(var k = 0; k < currentDtText.length;k++)
                    {
                        currentDtText[k] = currentDtText[k].replace(/^\s+/g, '');
                        cleanedArticle = GexfJS.params.activeArticle.replace(/^\s+/g, '');

                        if(currentDtText[k] ==  cleanedArticle)
                        {
                            debugger
                            _tagsMisEnValeur.push(_tix);
                ...
                //===============================================================================================================================
                //DRAW THE EDGES DEFINED, PROBABLY All
                if ((_isLinked || _showAllEdges) && (_ds.withinFrame || _dt.withinFrame) && _ds.visible && _dt.visible) {
                    ...
                    GexfJS.ctxGraphe.lineWidth      = (_isLinked ? 2:0.5);
        ...
        //===============================================================================================================================
        //SHOW NAME FOR THE NODES CONNECTED TO THE CENTRAL NODE
        for (var i in GexfJS.graph.nodeList) {
            var _d = GexfJS.graph.nodeList[i];
            if (_d.visible && _d.withinFrame) {
                if (i != _centralNode) {
        ...
        //===============================================================================================================================
        //DIFFERENT TEXT AND BORDER FOR THE CENTRAL NODE
        if (_centralNode != -1) {
